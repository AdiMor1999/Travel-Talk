...
afterAll(async () => {
  //Post.deleteMany({ city: "Test Post" });
...
describe("Post API test", () => {
  ...
  test("Test Post creation", async () => {
    ...
    expect(postsAfterCreation.body[0].photos).toEqual([
      "photo1.jpg",
      "photo2.jpg",
    ]);
  ...
  test("Test error handling in getById method when post is not found", async () => {
    const postId = new mongoose.Types.ObjectId().toHexString(); // Generate a random ObjectId
    const response = await request(app).get(`/post/${postId}`);
  ...
  test("Test Delete Post by ID when post id not found", async () => {
    const postId2 = "fkgkdk";
    const response = await request(app)
      .delete(`/post/${postId2}`)
      .set("authorization", `JWT ${accessToken}`);
